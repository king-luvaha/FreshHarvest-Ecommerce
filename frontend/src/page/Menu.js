...
const Menu = () => {
  ...
  // Ensure that productData is not empty and contains the product with the specified _id
  const productDisplay = productData.find((el) => el._id === filterby);

  
  // Check if productDisplay is available before accessing its properties
  if (!productDisplay) {
    ...
    <div className="p-2 md:p-4">
      <div className="w-full max-w-4xl m-auto md:flex bg-white">
        <div className="max-w-sm  overflow-hidden w-full p-5">
          {/* Check if productDisplay.image is available before rendering */}
